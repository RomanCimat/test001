...
fabrics_x = {
    ...
    "Ratier": { 3: 5.23, 4: 5.23, 5: 4.17}
}
frabics_jackquard = [4.63, 4.63, 5.23, 5.23, 4.17]
frabics_ratier = [-1, -1, 5.23, 5.23, 4.17]
...
#              (fabrics_x[loom][fabr] * hr * days) for fabr in fabrics)
#     <= capty[loom] for loom in inside_loom
# )
model.addConstr(
    quicksum(desicion_var[fabr, "Jacquard"]/(fabrics_x["Jacquard"][fabr]) for fabr in fabrics)
    <= capty["Jacquard"] * hr * days
)
model.addConstr(
...
funtion_objetive = quicksum(
     desicion_var[fabr, loom]*sale_price[fabr] - desicion_var[fabr, loom]*costs[loom][fabr]
